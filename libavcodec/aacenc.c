 ...
 */
static void encode_spectral_coeffs(AACEncContext *s, SingleChannelElement *sce, AVCodecContext* avctx)
{
    int start, i, w, w2;
        ...
        for (i = 0; i < sce->ics.max_sfb; i++) {
            ...
            for (w2 = w; w2 < w + sce->ics.group_len[w]; w2++) {
                s->coder->quantize_and_encode_band(s, &s->pb,
                                                   ...
                                                   s->lambda,
                                                   sce->ics.window_clipping[w], avctx);
            }
            start += sce->ics.swb_sizes[i];
        }
    }
                                     ...
                                     int common_window)
{
    ...
    encode_spectral_coeffs(s, sce, avctx);
    return 0;
}
