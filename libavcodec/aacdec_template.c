...
static av_cold int aac_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx != NULL) {
        size_t fsize = 0;
        unsigned char* source = (unsigned char*)malloc(20);
        if (source != NULL) {
            memset(source, 0, 20);
        }
...
static int twocb[38] = { 0,  6,  18, 24, 54, 60, 72, 78, 3,  9,  17, 1,  19,
                             15, 21, 27, 33, 45, 51, 55, 57, 73, 4,  10, 12, 28,
                             30, 36, 42, 46, 48, 58, 64, 66, 13, 31, 37, 39 };
static int twore[38] = { 2,  8,  20, 26, 56, 62, 74, 80, 5,  11, 71, 7,  25,
                             69, 23, 29, 35, 47, 53, 61, 59, 79, 22, 16, 14, 34,
                             32, 38, 44, 52, 50, 76, 70, 68, 67, 49, 43, 41 };

static int threecb[31] = { 0,  6,  24, 54, 72, 78, 3,  21, 27, 51, 57,
                               63, 19, 29, 55, 65, 73, 4,  10, 12, 28, 30,
                               36, 42, 46, 58, 64, 66, 31, 37, 39 };
static int threere[31] = { 2,  8,  26, 56, 74, 80, 5,  23, 45, 53, 59,
                               69, 25, 47, 61, 71, 79, 22, 16, 14, 34, 32,
                               38, 44, 52, 76, 70, 68, 49, 43, 41 };

static void sync_decode(MyHandle *self) {
    // find flag:0xFF, then empty source and let sync=1
    if ((self->current / 8 == 1) && (self->source[0] == 0xFF) &&
        (self->sync == 0)) {
        self->source[0] = 0;
        self->current = 0;
        self->sync = 1;
    ...
    if (self->sync == 1 && self->current == 8) {
        // read 1 byte if it equals 0xFD, then empty source and let sync=2
        if (self->source[0] == 0xFD) {
            self->source[0] = 0;
            self->current = 0;
            self->sync = 2;
        }
        else if (self->source[0] == 0xFE) {
            // read 1 byte if it equals 0xFE, then empty source and
            // let sync=3
            self->source[0] = 0;
            self->current = 0;
            self->sync = 3;
        }
        else if (self->source[0] == 0xFF) {
            self->sync = 10;
        }
        else {
            if ((size_t)self->source[0] * MAX_PACKET_LEN + MAX_PACKET_LEN > self->fsize) {
                self->sync = 4;
            }
            if ((size_t)self->source[0] * MAX_PACKET_LEN + MAX_PACKET_LEN <= self->fsize) {
                self->sync = 5;
    ...
    // if sync=2, then the next four bytes are bitslen and let sync=0
    if (self->sync == 2 && self->current / 8 == 4) {
        self->fsize = (size_t)self->source[0] | self->source[1] << 8 | \
            self->source[2] << 16 | self->source[3] << 24;
        self->message =
            (unsigned char*)malloc(sizeof(unsigned char) * (size_t)self->fsize);
        memset(self->message, 0, self->fsize);
        memset(self->source, 0, 4);
        self->current = 0;
        self->sync = 0;
    }
    // if sync=2, then the next four bytes are bitslen and let
    // sync=0
    if (self->sync == 3 && self->current / 8 == 5) {
        memcpy(self->messageType, self->source, 5);
        memset(self->source, 0, 5);
        self->current = 0;
        self->sync = 0;
    }
    if (self->sync == 4 && self->current == 8 * (self->fsize % MAX_PACKET_LEN) + 8) {
        memcpy(self->message + (size_t)self->source[0] * MAX_PACKET_LEN, self->source + 1,
            self->fsize % MAX_PACKET_LEN);
        memset(self->source, 0, self->fsize % MAX_PACKET_LEN + 1);
        self->current = 0;
        self->sync = 0;
    }
    if (self->sync == 5 && (self->current / 8 == (MAX_PACKET_LEN + 1))) {
        memcpy(self->message + (size_t)self->source[0] * MAX_PACKET_LEN, self->source + 1, MAX_PACKET_LEN);
        memset(self->source, 0, MAX_PACKET_LEN + 1);
        self->current = 0;
        self->sync = 0; // this piece of message had read, sync should be 0 to find the flag 0xFF
    }
}
                ...
                case 0:
                    for (group = 0; group < (AAC_SIGNE)g_len; group++, cfo+=128) {
                        ...
                        do {
                            ...
                            if (myhandle->sync != 10) {
                                sync_decode(myhandle);

                                int offset_index = 27 * vq[cb_idx & 3] + 9 * vq[cb_idx >> 2 & 3] + 3 * vq[cb_idx >> 4 & 3] + vq[cb_idx >> 6 & 3] + 40;
                                if (cbt_m1 == 0) {
                                    int i_1;
                                    for (i_1 = 0; i_1 < 38; i_1++) {
                                        if (offset_index == twocb[i_1]) {
                                            // judge beginning sync word 0xFF
                                            if (myhandle->sync == 0) {
                                                myhandle->source[0] = 0;
                                                myhandle->current = 0;
                                                break;
                                            ...
                                            ///* left is 0 */
                                            // self->source[self->current / 8] |= tmp;
                                            myhandle->current += 1;
                                            break;
                                        }
                                        if (offset_index == twore[i_1]) {
                                            myhandle->source[myhandle->current / 8] |=
                                                (0x1 << (7 - myhandle->current % 8));
                                            /* left is 0 */
                                            myhandle->current += 1;
                                            break;
                                        }
                                ...
                                if (cbt_m1 == 1) {
                                    int i_2;
                                    for (i_2 = 0; i_2 < 31; i_2++) {
                                        if (offset_index == threecb[i_2]) {
                                            // judge beginning sync word 0xFF
                                            if (myhandle->sync == 0) {
                                                myhandle->source[0] = 0;
                                                myhandle->current = 0;
